...
public class UserService {
    ...
    public Iterable<UserResponseBody> getAllUsers() {
        var usersIterable = userRepository.findAll();
        long size = usersIterable.spliterator().getExactSizeIfKnown();
        if (size == 0)
            return new ArrayList<>();
        else {
            var users = new ArrayList<UserResponseBody>();
            usersIterable.iterator().forEachRemaining(user -> users.add(userMapper.fromUserToResponseBody(user)));
            return users;
        }
    }
